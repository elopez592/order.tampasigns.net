...
def test_storefront_category_metadata_and_duplicates(env):
    ...
    assert set(products['Transfer stickers']['config']['storefront_categories']) == {'Vehicle Signage', 'Stickers'}
...
def test_decals_usdot_and_apparel_listings(env):
    ...
    assert [x['label'] for x in usdot['config']['size_options']] == ['18 x 4 in', '24 x 6 in', '36 x 8 in']

    for name in ('DTF transfers', 'Embroidered hats', 'Embroidered polos'):
        assert products[name]['config']['storefront_categories'] == ['Apparel']
        assert products[name]['config']['instant'] is False
...
def test_usdot_quote_preserves_generated_preview_copy(env):
    ...
    response = client.post('/api/calculate', json={'items':[{
        'product_id': usdot['id'], 'width': 18, 'height': 4, 'quantity': 2,
