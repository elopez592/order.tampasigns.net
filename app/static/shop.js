...
export function createShop(ctx) {
  const {state,app,api,esc,money,input,select,formFooter,showModal,closeModal,toast,publicHeader,publicProductName,productPath,setPublicSeo,storefrontProductsFor,productIcon,productImage,calculatorView,recalculate,currentItems,orderModal,actions,forms}=ctx;
  ...
  function allProducts(){setPublicSeo();const products=state.catalog.products.filter(p=>!(p.name.toLowerCase()==='vehicle wraps'&&state.catalog.products.some(x=>x.name.toLowerCase()==='partial vehicle wraps')));page('All products',`<p class="muted mb">Everything we make, in one place.</p><div class="all-products-grid">${products.map(p=>`<article class="panel product-catalog-card"><a class="catalog-image-link" href="${productPath(p)}" aria-label="View ${esc(publicProductName(p))}">${productImage(p,'catalog')}</a><div class="eyebrow">${esc((p.config.storefront_categories||[]).join(' / '))}</div><h2 class="mt"><a href="${productPath(p)}">${esc(publicProductName(p))}</a></h2><p class="muted mt">${esc(p.config.description)}</p><a class="btn primary mt" href="${productPath(p)}" data-action="browse-product" data-id="${p.id}">Customize & add to project</a></article>`).join('')}</div>`);}
  ...
  return {count:()=>project.length,setupProduct,shirtItem,designFiles,resumeCanva,clear:()=>{project=[];persist();state.projectCheckout=false;},route:async path=>{if(path==='/products')return allProducts();if(path==='/project')return projectView();if(path==='/account')return accountView();},handles:path=>['/products','/project','/account'].includes(path)};
}
