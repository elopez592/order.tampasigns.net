...
export function createShop(ctx) {
  ...
  const photoArtworkRect=canvas=>{
    if(!photoMockup.artwork)return null;
  ...
  function drawPhotoMockup(editing=true){
    ...
    if(photoMockup.background){
      ...
      ctx.fillStyle='#667780';ctx.font='20px Arial';ctx.textAlign='center';ctx.fillText('1. Upload your storefront or vehicle photo',canvas.width/2,canvas.height/2);
    }
    if(photoMockup.artwork){
      const r=photoArtworkRect(canvas);
      ctx.save();ctx.globalAlpha=photoMockup.opacity/100;ctx.drawImage(photoMockup.artwork,r.x,r.y,r.w,r.h);ctx.restore();
      if(editing){
        ctx.save();ctx.strokeStyle='#ffffff';ctx.lineWidth=3;ctx.setLineDash([9,7]);ctx.strokeRect(r.x,r.y,r.w,r.h);ctx.setLineDash([]);
        ctx.fillStyle='#ffffff';ctx.strokeStyle='#263638';ctx.lineWidth=2;ctx.fillRect(r.x+r.w-10,r.y+r.h-10,20,20);ctx.strokeRect(r.x+r.w-10,r.y+r.h-10,20,20);ctx.restore();
      }
    }
  }
  function showPhotoMockup(){
    ...
    showModal('Preview artwork on your photo',`<div class="photo-mockup-steps"><label class="field"><span><strong>1.</strong> Upload your photo</span><input id="photo-mockup-bg" type="file" accept="image/png,image/jpeg"></label><label class="field"><span><strong>2.</strong> Upload your logo or artwork</span><input id="photo-mockup-art" type="file" accept="image/png,image/jpeg"></label></div><div class="photo-mockup-stage"><canvas id="photo-mockup-canvas" class="photo-mockup-canvas" width="900" height="600"></canvas><p class="photo-mockup-tip">Drag the artwork to position it. Drag the white corner handle to resize it.</p></div><details class="photo-mockup-adjustments"><summary>Optional adjustments</summary><label class="field mt"><span>Artwork opacity</span><input id="photo-mockup-opacity" type="range" min="20" max="100" value="92"></label></details><p class="field-hint mt">Reference mockup only. Final scale, crop, color and placement are confirmed in your production proof.</p><div class="modal-footer"><button type="button" class="btn light" data-action="close">Cancel</button><button type="button" class="btn primary" data-action="photo-mockup-save">Save mockup to Project</button></div>`,true);
    ...
    $('#photo-mockup-opacity').oninput=e=>{photoMockup.opacity=Number(e.target.value);drawPhotoMockup();};
    const canvas=$('#photo-mockup-canvas');
    let dragMode='',offsetX=0,offsetY=0;
    const pointer=event=>{const rect=canvas.getBoundingClientRect();return {x:(event.clientX-rect.left)*canvas.width/rect.width,y:(event.clientY-rect.top)*canvas.height/rect.height};};
    canvas.onpointerdown=event=>{
      if(!photoMockup.artwork)return;
      const p=pointer(event),r=photoArtworkRect(canvas),handleX=r.x+r.w,handleY=r.y+r.h;
      if(Math.hypot(p.x-handleX,p.y-handleY)<=28)dragMode='resize';
      else if(p.x>=r.x&&p.x<=r.x+r.w&&p.y>=r.y&&p.y<=r.y+r.h){dragMode='move';offsetX=p.x-r.cx;offsetY=p.y-r.cy;}
      else return;
      canvas.setPointerCapture(event.pointerId);event.preventDefault();
    };
    canvas.onpointermove=event=>{
      if(!dragMode||!photoMockup.artwork)return;
      const p=pointer(event),r=photoArtworkRect(canvas);
      if(dragMode==='move'){
        photoMockup.x=Math.max(0,Math.min(100,(p.x-offsetX)/canvas.width*100));
        photoMockup.y=Math.max(0,Math.min(100,(p.y-offsetY)/canvas.height*100));
      }else{
        const width=Math.abs(p.x-r.cx)*2/canvas.width*100;
        photoMockup.scale=Math.max(8,Math.min(100,width));
      }
      drawPhotoMockup();event.preventDefault();
    };
    const stop=()=>{dragMode='';};
    canvas.onpointerup=stop;canvas.onpointercancel=stop;canvas.onpointerleave=event=>{if(!canvas.hasPointerCapture?.(event.pointerId))stop();};
    drawPhotoMockup();
  }
  ...
  function showProductFinder(){
    showModal('Help me choose',`<form data-form="product-finder" class="stack finder-wizard"><div class="finder-step"><span>1</span><div>${select('surface','What are you branding?',[['storefront','Storefront / windows'],['vehicle','Car, truck or van'],['fleet','Multiple vehicles'],['construction','Job site / construction'],['event','Event / temporary promotion'],['apparel','Shirts, polos, hats or hoodies'],['sign','General sign / rigid panel'],['sticker','Stickers / decals']],'storefront')}</div></div><div class="finder-step"><span>2</span><div>${select('use_case','Where / how will it be used?',[['outdoor','Outdoors / long-term'],['indoor','Indoors'],['temporary','Temporary promotion / event'],['mobile','On a moving vehicle'],['mixed','A mix / not sure']],'outdoor')}</div></div><div class="finder-step"><span>3</span><div>${select('help','What help do you need?',[['ready','I have print-ready artwork'],['design','I need design help'],['install','I need installation help'],['unsure','I am not sure yet']],'ready')}</div></div><label class="field"><span>Anything important we should consider?</span><textarea name="notes" maxlength="1000" placeholder="Multiple locations, deadline, unusual surface, special access, etc."></textarea></label>${formFooter('Show my recommendation')}</form>`);
  }
  async function accountView(){
    ...
    const profile=r.profile||{},assets=r.saved_assets||[],company=profile.company||'No company saved';
    const savedBits=[profile.locations&&'Locations',profile.fleet_notes&&'Fleet',profile.brand_notes&&'Brand notes',profile.po_number&&'PO'].filter(Boolean);
    page('Your account',`<section class="account-hero panel"><div><div class="eyebrow">CUSTOMER WORKSPACE</div><h2>${esc(state.customer.name)}</h2><p class="muted">${esc(state.customer.email)}</p></div><div class="row wrap"><button class="btn ghost" data-action="customer-logout">Sign out</button></div></section>
      <div class="account-dashboard mt">
        <section class="account-projects"><div class="row between wrap mb"><div><div class="eyebrow">MY PROJECTS</div><h2>Projects & reorders</h2></div></div>${r.orders.length?r.orders.map(o=>`<article class="panel account-project-card"><div class="account-project-main"><div><span class="badge">${esc(String(o.stage||'received').replaceAll('_',' '))}</span><h3>${esc(o.number||('JOB-'+String(o.id).padStart(4,'0')))} · ${esc(o.title)}</h3><p class="muted">${o.due_date?'Requested '+esc(o.due_date):'No requested date'}${o.priority==='rush'?' · Rush requested':''}</p></div><div class="account-project-actions"><button class="btn primary" data-action="customer-order" data-id="${o.id}">View project</button><button class="btn light" data-action="customer-reorder" data-id="${o.id}">Reorder</button></div></div></article>`).join(''):'<section class="panel account-empty"><h3>No submitted projects yet</h3><p class="muted mt-sm">Projects submitted while signed in will appear here for status, artwork and reorders.</p><a class="btn primary mt" href="/products">Browse products</a></section>'}</section>
        <aside class="stack"><section class="panel"><div class="eyebrow">SAVED ASSETS</div><h3 class="mt-sm">Artwork library</h3><p class="field-hint mt-sm">Files from submitted projects stay available for reference and reorders.</p>${assets.length?assets.slice(0,8).map(a=>`<a class="file-row" href="/api/customer/assets/${a.id}" target="_blank" rel="noopener"><div class="grow"><strong>${esc(a.filename)}</strong><div class="muted tiny">${esc(a.job_number)} · ${(a.size/1048576).toFixed(2)} MB</div></div><span>↗</span></a>`).join(''):'<p class="muted mt">No saved artwork yet.</p>'}${assets.length>8?'<p class="tiny muted mt">Showing your most recent 8 files.</p>':''}</section>
        <section class="panel account-business-summary"><div class="eyebrow">BUSINESS SETTINGS</div><h3 class="mt-sm">${esc(company)}</h3><p class="field-hint mt-sm">${savedBits.length?esc(savedBits.join(' · ')):'Add company, location, fleet and brand details to speed up repeat orders.'}</p></section></aside>
      </div>
      <details class="panel business-profile-details mt"><summary><span><strong>Business account settings</strong><small>Company, locations, fleet, brand standards, PO and authorized buyers</small></span><span class="link">Edit business profile</span></summary><form data-form="customer-profile" class="stack mt">${select('account_type','Account type',[['standard','Standard customer'],['commercial','Commercial / multi-project']],profile.account_type||'standard')}<div class="fields">${input('company','Company / organization',profile.company||'','text','maxlength="160"')}${input('po_number','Default PO number (optional)',profile.po_number||'','text','maxlength="120"')}</div><label class="field"><span>Locations / properties</span><textarea name="locations" maxlength="4000" placeholder="Store names, addresses, unit numbers, property notes...">${esc(profile.locations||'')}</textarea></label><label class="field"><span>Fleet / vehicles</span><textarea name="fleet_notes" maxlength="4000" placeholder="Vehicle numbers, year/make/model, assigned locations...">${esc(profile.fleet_notes||'')}</textarea></label><label class="field"><span>Brand standards / saved instructions</span><textarea name="brand_notes" maxlength="4000" placeholder="Brand colors, approved logo notes, repeat production instructions...">${esc(profile.brand_notes||'')}</textarea></label><div class="fields"><label class="field"><span>Authorized buyers / contacts</span><textarea name="authorized_buyers" maxlength="2000">${esc(profile.authorized_buyers||'')}</textarea></label><label class="field"><span>Tax-exempt certificate / reference</span><textarea name="tax_exempt_note" maxlength="1000">${esc(profile.tax_exempt_note||'')}</textarea></label></div><p class="field-hint">Tax-exempt information is kept as a shop reference and does not automatically remove tax.</p>${formFooter('Save business settings')}</form></details>`);
  }
  ...
  forms['product-finder']=async(f,d)=>{
    ...
    const helpCopy={ready:'Start with the product options below and attach your print-ready artwork.',design:'Start with the product category, then use Request design help so we can quote the artwork.',install:'Start with the product category and note that installation is needed. We will confirm site details and scheduling.',unsure:'This is the best product family to start with. You can still ask us to review the project before anything moves to production.'};
    state.selectedCategory=category;sessionStorage.setItem('storefront_category',category);closeModal();
    showModal('Your best starting point',`<div class="finder-result"><div class="eyebrow">RECOMMENDATION</div><h2>${esc(category)}</h2><p>${esc(helpCopy[d.help]||helpCopy.unsure)}</p><div class="finder-result-meta"><span>${esc(d.use_case.replaceAll('_',' '))}</span><span>${esc(d.help.replaceAll('_',' '))}</span></div>${d.notes?`<p class="field-hint">You noted: ${esc(d.notes)}</p>`:''}<div class="row wrap mt"><a class="btn primary" href="/">Start with ${esc(category)}</a>${d.help==='design'?'<button class="btn light" data-action="design-quote">Request design help</button>':''}${d.help==='unsure'?'<button class="btn light" data-action="custom-quote">Ask for a custom review</button>':''}</div></div>`);
  };
